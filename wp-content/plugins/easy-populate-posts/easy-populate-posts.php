...
define( 'SPP_PLUGIN_VERSION', 3.61 );
...
class SISANU_Popupate_Posts {
	...
	private function init() {
		...
		add_action( 'init', [ $class, 'load_plugin_settings' ] );
		...
		add_action( 'wp_ajax_spp-plugin-deactivate-notice', [ $class, 'plugin_admin_notices_cleanup' ] );
		add_action( 'plugins_loaded', [ $class, 'plugin_ver_check' ] );
		add_action( 'added_post_meta', [ $class, 'reset_spp_meta_list' ], 10, 4 );
		add_action( 'updated_post_meta', [ $class, 'reset_spp_meta_list' ], 10, 4 );
		add_action( 'deleted_post_meta', [ $class, 'reset_spp_meta_list' ], 10, 4 );
	}
	...
	public static function get_post_meta_keys() { //phpcs:ignore
		...
		if ( false === $list ) {
			$acf_fields = $wpdb->get_results( $wpdb->prepare( // phpcs:ignore
			...
			$result = $wpdb->get_col( $wpdb->prepare( // phpcs:ignore
			...
			if ( ! empty( $acf_fields ) ) {
				...
				foreach ( $acf_fields as $item ) {
					...
					$result = array_diff( $result, [ $item->post_excerpt, '_' . $item->post_excerpt ] );
				}
				$list .= '</optgroup>';
			}
			if ( ! empty( $result ) ) {
	 ...
	 * @return void
	 */
	public static function reset_spp_meta_list( $meta_id, $post_id, $meta_key, $meta_value ) { // phpcs:ignore
						...
						<td colspan="5">
							...
							<table class="spp-table fixed" width="100%">
								...
								<tr>
									...
									<td><?php echo self::get_post_meta_keys(); // phpcs:ignore ?></td>
								</tr>
							</table>
						</td>
					</tr>
	...
	public static function get_random_tags( $tags_list = '' ) { // phpcs:ignore
		...
		if ( ! empty( $tags_list ) ) {
			...
			if ( 1 === $total_tags ) {
				...
				shuffle( $t );
				$tags = array_slice( $t, 0, wp_rand( 1, count( $t ) - 1 ) );
			}
		}
		return $tags;
	}
	...
	public static function execute_add_random_posts() {
		...
		for ( $i = 0; $i < self::$settings['max_number']; $i ++ ) {
			...
			if ( ! empty( $name ) ) {
				$max_blocks  = ( 0 === (int) self::$settings['content_p'] ) ? wp_rand( 1, 6 ) : self::$settings['content_p'];
	...
	public static function plugin_admin_notices() {
		...
		if ( ! empty( $maybe_trans ) ) {
			$selector  = '.notice.' . esc_attr( self::PLUGIN_TRANSIENT );
			...
			$other_notice = sprintf(
				...
				__( 'Check out my other <a href="%1$s" target="_blank">%2$s free plugins</a> on WordPress.org and the <a href="%3$s" target="_blank">%4$s premium extensions</a> available!', 'spp' ),
			...
			<style>
				<?php echo esc_attr( $selector ); ?> { width:100%; max-width:100%; overflow-x: hidden; border-left-color:rgb(250,203,53); background:rgba(250,203,53,0.2); border-color: rgb(250,203,53); box-sizing: border-box; padding: 0; border-left-width: 20px;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-grid { display:grid; grid-template-columns:repeat( auto-fit, minmax(580px, 1fr) ); grid-gap:0px; justify-content:space-between; width:100%; max-width:100%; overflow-x: hidden; align-content: center; padding: 0; align-items: stretch;}
				<?php echo esc_attr( $selector ); ?> a {color: #013087; text-decoration: none;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-grid > div { box-sizing: border-box;display: flex; align-content: center; align-items: center; padding:10px 20px; }
				<?php echo esc_attr( $selector ); ?> .notice-plugin-title { display: grid !important; flex-direction: row; grid-template-columns: 128px 1fr;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-title h3 {margin: 10px 0;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-icon {width: 128px; margin-left:-20px; margin-top:-10px; margin-bottom:-10px; overflow: hidden;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-icon img {display: block; margin:0;}
				<?php echo esc_attr( $selector ); ?> .notice-plugin-donate {display:block; margin-top: 10px}
			</style>
			...
			<div class="updated notice is-dismissible <?php echo esc_attr( self::PLUGIN_TRANSIENT ); ?>"
				...
				<div class="notice-plugin-grid">
					<div class="notice-plugin-title">
						<div class="notice-plugin-icon">
							<a href="<?php echo esc_url( self::$plugin_url ); ?>"><img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) . '/assets/images/icon-128x128.gif' ); ?>"></a>
						</div>
						<div>
							<h3>
								<?php
					...
					<div class="notice-plugin-suport">
						<div>
							<?php
							echo wp_kses_post( sprintf(
								// Translators: %1$s - image URL, %2$s - icon URL, %3$s - donate URL, %4$s - link style, %5$s - icon style, %6$s - rating.
								__( 'This plugin is free to use, but not to operate. Please consider supporting my services by making a <a href="%1$s" target="_blank">donation</a>. It would make me very happy if you would leave a %2$s rating. %3$s', 'spp' ),
								$donate,
								'<a href="' . self::PLUGIN_SUPPORT_URL . 'reviews/?rate=5#new-post" class="rating" target="_blank" title="' . esc_attr__( 'A huge thanks in advance!', 'spp' ) . '">★★★★★</a>',
								__( 'A huge thanks in advance!', 'spp' )
							) );
							?>
							<a class="notice-plugin-donate" href="<?php echo esc_url( $donate ); ?>" target="_blank"><img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) . '/assets/images/buy-me-a-coffee.png?v=' . SPP_PLUGIN_VERSION ); ?>" width="280"></a>
						</div>
					</div>
				</div>
			</div>
...
register_activation_hook( __FILE__, [ $sisanu_popupate_posts, 'activate_plugin' ] );
register_deactivation_hook( __FILE__, [ $sisanu_popupate_posts, 'deactivate_plugin' ] );
